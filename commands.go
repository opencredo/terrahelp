...
func encryptCommand(f func(provider string) *terrahelp.CryptoHandler) cli.Command {
	...
	command := cli.Command{
		...
		Description: "Using either the 'simple' (default) or 'vault' provider, encrypt will ensure that the relevant content\n" +
			...
			"   The desired 'provider' and well as 'mode' can be supplied as CLI arguments, via the TH_ENCRYPTION_PROVIDER \n" +
			"   and TH_ENCRYPTION_MODE environment variables, or via a JSON file specified by the -config-file flag. Encrypted\n" +
			"   values will always conform to the following format: \n @terrahelp-encrypted(ENCRYPTED_CONTENT). Decryption \n" +
			"   essentially operates in reverse.\n" +
		...
		Flags: []cli.Flag{
			cli.StringFlag{
				Name:      "config-file",
				Usage:     "JSON file from which to load values for other flags",
				EnvVar:    "TH_CONFIG_FILE",
				TakesFile: true,
			},
	...
	command.Before = func(c *cli.Context) error {
		if c.String("config-file") != "" {
			err := altsrc.InitInputSourceWithContext(command.Flags, altsrc.NewJSONSourceFromFlagFunc("config-file"))(c)
			exitIfError(err)
		}
		return nil
	}
...
func decryptCommand(f func(provider string) *terrahelp.CryptoHandler) cli.Command {
	...
	command := cli.Command{
		...
		Description: "Using either the 'simple' (default) or 'vault' provider, decrypt will ensure that the against content\n" +
			...
			"   via the TH_ENCRYPTION_PROVIDER and TH_ENCRYPTION_MODE environment variables, or via a JSON file specified\n" +
			"   by the -config-file flag. \n\n" +
			"   EXIT STATUS \n" +
			"   ----------- \n" +
